...
public class AdvancedSwerveKinematics {
  ...
  public static ChassisSpeeds correctForDynamics(ChassisSpeeds requestedSpeeds) {
    Pose2d futureRobotPose =
        new Pose2d(
            requestedSpeeds.vxMetersPerSecond * RobotConstants.ROBOT_LOOP_PERIOD,
            requestedSpeeds.vyMetersPerSecond * RobotConstants.ROBOT_LOOP_PERIOD,
            Rotation2d.fromRadians(
                requestedSpeeds.omegaRadiansPerSecond * RobotConstants.ROBOT_LOOP_PERIOD));

    Twist2d twistForPose = log(futureRobotPose);

    ChassisSpeeds correctedSpeeds =
        new ChassisSpeeds(
            twistForPose.dx / RobotConstants.ROBOT_LOOP_PERIOD,
            twistForPose.dy / RobotConstants.ROBOT_LOOP_PERIOD,
            twistForPose.dtheta / RobotConstants.ROBOT_LOOP_PERIOD);
  ...
  public SwerveModuleState[] toSwerveModuleStates(
    ...
    firstOrderInputMatrix.set(0, 0, xSpeed);
    firstOrderInputMatrix.set(1, 0, ySpeed);
    firstOrderInputMatrix.set(2, 0, rotSpeed);

    secondOrderInputMatrix.set(2, 0, Math.pow(rotSpeed, 2));
    ...
    SwerveModuleState[] swerveModuleStates =
        new SwerveModuleState[RobotConstants.moduleLocations.length];
    double[] moduleTurnSpeeds = new double[RobotConstants.moduleLocations.length];

    for (int i = 0; i < RobotConstants.moduleLocations.length; i++) {
      // Angle that the module location vector makes with respect to the robot
      Rotation2d moduleAngle =
          new Rotation2d(
              Math.atan2(
                  RobotConstants.moduleLocations[i].getY(),
                  RobotConstants.moduleLocations[i].getX()));
      ...
      double moduleX =
          RobotConstants.moduleLocations[i].getNorm() * Math.cos(moduleAngle.getRadians());
      double moduleY =
          RobotConstants.moduleLocations[i].getNorm() * Math.sin(moduleAngle.getRadians());
      // -r_y
      ...
      moduleTurnSpeeds[i] = secondOrderOutput.get(1, 0) / moduleSpeed - rotSpeed;
    }
